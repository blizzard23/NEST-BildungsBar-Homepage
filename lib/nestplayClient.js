...
/* Link direkt in den Bearbeitungsmodus des Spiels auf nest-play.de.
   Ist man nicht eingeloggt, zeigt NESTplay sein Login-Fenster; über den
   redirect-Parameter landet man nach dem Login direkt in der Bearbeitung. */
export function gameEditUrl(game) {
  const ziel = `/edit/${game.id}`;
  return `${NESTPLAY_URL}${ziel}?redirect=${encodeURIComponent(ziel)}`;
}
